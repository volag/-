        ...
        <div :class="advanced ? null: 'fold'">
            ...
            <a-col :md="8" :sm="24" >
              <a-form-item
                ...
                :labelCol="{span: 4}"
                :wrapperCol="{span: 18, offset: 2}">
                <a-input v-model="queryParams.signedAddress"/>
              </a-form-item>
            </a-col>
          <template v-if="advanced">
            <a-col :md="8" :sm="24" >
              <a-form-item
                ...
                :labelCol="{span: 4}"
                :wrapperCol="{span: 18, offset: 2}">
                <range-date @change="handleSignedTImeChange" ref="signedTime"></range-date>
              </a-form-item>
            </a-col>
          </template>
          <template v-if="advanced">
    ...
    <div>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          <a-icon type="setting" theme="twoTone" twoToneColor="#4a9ff5" @click="edit(record)" title="修改合同"></a-icon>
          &nbsp;
...
export default {
    name: 'ContractManage',
    // components: {contractInfo, contractAdd, contractEdit, DeptInputTree, RangeDate},
    components: {ContractInfo,ContractEdit,ContractAdd,RangeDate},
    data () {
    return {
      advanced: false,
      contractInfo: {
  ...
  computed: {
    columns () {
      ...
      return [{
        ...
        sorter: true,
        sortOrder: sortedInfo.columnKey === 'contractName' && sortedInfo.order
      }, 
      // {
      //   title: '邮箱',
      //   dataIndex: 'email',
      ...
      },
      {
        ...
        sorter: true,
        sortOrder: sortedInfo.columnKey === 'finishTime' && sortedInfo.order
      },
      {
        title: '开票日期',
        dataIndex: 'makeInvoiceTime',
        sorter: true,
        sortOrder: sortedInfo.columnKey === 'makeInvoiceTime' && sortedInfo.order
      },
      {
        title: '支付方式',
        dataIndex: 'payWay',
  ...
  methods: {
    ...
    handleSignedTImeChange (value) {
      if (value) {
        this.queryParams.signedTimeFrom = value[0]
        this.queryParams.signedTimeTo = value[1]
      }
    },
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          for (let key of that.selectedRowKeys) {
            contractIds.push(that.dataSource[key].contractId)
          }
          that.$delete('contract/' + contractIds.join(',')).then(() => {
