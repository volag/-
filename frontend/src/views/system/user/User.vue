      ...
      <a-form layout="horizontal">
        <a-row>
          <div :class="advanced ? null: 'fold'">
            ...
            <a-col :md="12" :sm="24">
              <a-form-item label="公司名称" :labelCol="{span: 4}" :wrapperCol="{span: 18, offset: 2}">
                
                <a-select :size="size" defaultValue style="width: 350px;" @change="onCompanyChange">
                  <a-select-option v-for="(item,index) of companyTree" :key="index">{{item}}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
  ...
  methods: {
    companyName() {
      this.$get("company").then(r => {
        let companyList = r.data.rows;
        companyList.forEach(item => {
          this.companyTree.push(item.companyName);
        });
      });
    },
    ...
    onCompanyChange(value) {
      this.queryParams.companyName = this.companyTree[value] || "";
    },
